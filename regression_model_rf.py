...
def rf_classifier(feat):
    ...
    print(time.time()-t)
    with open('../hyperparameter_file_rf_regression.csv','a') as ref:
        ref.write(str(max_features)+','+str(n_estimators)+','+str(train_pred)+','+str(valid_pred)+','+str(mean_squared_error(y_train,rf.predict(X_train)))+','+str(mean_squared_error(y_valid,rf.predict(X_valid)))+'\n')
