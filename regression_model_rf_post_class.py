...
def rf_classifier(feat):
    ...
    for i in ctf:
        ...
        with open('../hyperparameter_file_rf_regression_post_class.csv','a') as ref:
            ref.write(str(i)+','+str(max_features)+','+str(n_estimators)+','+str(train_pred)+','+str(valid_pred)+','+str(mean_squared_error(y_train,y_pred_train))+','+str(mean_squared_error(y_valid,y_pred_valid))+'\n')
